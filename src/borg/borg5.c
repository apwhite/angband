...
static void borg_update_kill_new(int i)
{
    ...
    for (k = 0; k < 32; k++)
    {
        if (r_ptr->spell_flags[0] & (1L << k)) kill->spell[num++] = k + 32 ;
    }

    /* Extract the "normal" spells */
    for (k = 0; k < 32; k++)
    {
        if (r_ptr->spell_flags[1] & (1L << k)) kill->spell[num++] = k + 32 * 2;
    }

    /* Extract the "bizarre" spells */
    for (k = 0; k < 32; k++)
    {
        if (r_ptr->spell_flags[2] & (1L << k)) kill->spell[num++] = k + 32 * 3;
    }
...
static int borg_locate_kill(cptr who, int y, int x, int r)
{
    ...
    if (rf_has(r_ptr->flags, RF_CHAR_CLEAR) ||(rf_has(r_ptr->flags, RF_CHAR_MULTI)))
    {
        /* Note */
...
void borg_init_5(void)
{
    ...
    /* Allocate a buffer */
	borg_msg_buf = C_ZNEW(borg_msg_siz, char);
	...
	borg_msg_pos = C_ZNEW(borg_msg_max, s16b);

    /* Allocate array of use-types */
	borg_msg_use = C_ZNEW(borg_msg_max, s16b);


    /*** Object/Monster tracking ***/

    /* Array of "wanks" */
	borg_wanks = C_ZNEW(AUTO_VIEW_MAX, borg_wank);
    ...
    /* Allocate the arrays */
	borg_unique_text = C_ZNEW(borg_unique_size, cptr);
	borg_unique_what = C_ZNEW(borg_unique_size, s16b);
    ...
    /* Allocate the arrays */
	borg_normal_text = C_ZNEW(borg_normal_size, cptr);
	borg_normal_what = C_ZNEW(borg_normal_size, s16b);
