...
void borg_money_scum(void)
{
	...
	if (borg_money_scum_amount == 0) return;
    ...
    *b_home_power = borg_power_home();
    ...
    for (n = 0; n < STORE_INVEN_MAX; n++)
    {
        ...
        for (i = 0; i < INVEN_MAX_PACK; i++)
        {
			...
			if (bought_item_tval == item->tval && bought_item_sval == item->sval && bought_item_pval== item->pval && bought_item_store == 7) continue;
...
static bool borg_good_sell(borg_item *item, int who)
{
    ...
    if ((op_ptr->opt[OPT_birth_randarts] || op_ptr->opt[OPT_adult_randarts]) && !item->fully_identified && item->name1)
    {
              /* CHECK THE ARTIFACTS */
...
static bool borg_good_buy(borg_item *item, int who)
{
			   ...
			   item->sval == SV_SCROLL_TELEPORT)))
		{
...

			{
			    ...
			    if (adj_dex_safe[borg_skill[BI_DEX]] + borg_skill[BI_CLEVEL] > save_throw ) /* Good chance to thwart mugging */
			    {
					...
					/* borg_money_scum_item = item->desc; */
				}
			}
    ...
    /* do not buy the item if I just sold it. */
    if (sold_item_tval == item->tval && sold_item_sval == item->sval && sold_item_store == who)
    {
#if 0
...
static bool borg_think_shop_buy_aux(void)
{
    ...
    for (k = 0; k < (MAX_STORES -1 ); k++)
    {
		...
		if (borg_money_scum_amount &&
		    (k != borg_money_scum_who && k != 0 && k != 7)) continue;
        ...
        for (n = 0; n < STORE_INVEN_MAX; n++)
        {
			...
			if (item->tval == TV_LIGHT && item->sval == SV_LIGHT_TORCH &&
				of_has(borg_items[INVEN_LIGHT].flags, OF_LIGHT_CURSE))
			{
				slot = -1;
			}
...
static bool borg_think_home_grab_aux(void)
{
    ...
    for (n = 0; n < STORE_INVEN_MAX; n++)
    {
        ...
        /* skip stuff if we sold bought it */
        if (sold_item_tval == item->tval && sold_item_sval == item->sval && sold_item_store == 7) continue;
		...
		borg_skill[BI_CDEPTH] >= 1)
    {
		...
		for (ii = 0; ii < 8; ii++)
		{
				...
				(y == c_y && x == c_x))
			{
				...
				if (cave_feat[c_y][c_x] == CAVE_GLOW) ag->info |= BORG_GLOW;
				continue;
			}
...
/* XXX Should we allow him great flexibility in retreiving loot? (not always safe?)*/
         ...
         borg_skill[BI_CDEPTH] < 30))
    {
             ...
             borg_skill[BI_CDEPTH] < 30)
		{
			...
			/* Flow to DownStair */
        	if (borg_flow_stair_more(GOAL_FLEE, FALSE)) return (TRUE);
...
/* XXX Should we allow him great flexibility in retreiving loot? (not always safe?)*/
         ...
         borg_skill[BI_CDEPTH] < 30))
    {
             ...
             borg_skill[BI_CDEPTH] < 30)
		{
			...
			/* Flow to DownStair */
        	if (borg_flow_stair_more(GOAL_FLEE, TRUE)) return (TRUE);
...
/* XXX Should we allow him great flexibility in retreiving loot? (not always safe?)*/
    ...
    /* Going down */
    if ((track_more_num && borg_skill[BI_CDEPTH] < 8 && safe_place) ||
        ag->feat == FEAT_MORE)
    {
        int y, x;
		int closeness = 8;
...
bool borg_think_dungeon(void)
{
        ...
        (cptr)NULL == borg_prepared(borg_skill[BI_CDEPTH] + 5) && !stair_less)
    {
        ...
        if (stair_more && borg_flow_stair_more(GOAL_BORE, TRUE))
        {
			/* Leave a note */
