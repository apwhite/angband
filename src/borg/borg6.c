            ...
            borg_read_scroll(SV_SCROLL_WORD_OF_RECALL))
        {
			...
			if (borg_skill[BI_CDEPTH] < borg_skill[BI_MAXDEPTH] &&
			    ((borg_skill[BI_MAXDEPTH] >= 60 &&
			     ...
			     (borg_skill[BI_CLEVEL] < 48 &&
			      borg_skill[BI_CDEPTH] >= borg_skill[BI_MAXDEPTH] - 3)))
		    {
...
static bool borg_heal(int danger )
{
    ...
    if (borg_skill[BI_ISBLIND] && (randint1(100) < 85))
    {
   ...
   /*  Hack -- rest until healed */
    if ( (!borg_skill[BI_ISBLIND] && !borg_skill[BI_ISPOISONED] && !borg_skill[BI_ISCUT] &&
         ...
         borg_check_rest(c_y, c_x) && !scaryguy_on_level && goal != GOAL_RECOVER)
    {
    ...
    if (borg_skill[BI_CURSP] < (borg_skill[BI_MAXSP] / 5) && (randint1(100) < 50))
    {
     ...
     * borg_escape() routine.
     */
    if (hp_down >= 650 && danger > borg_skill[BI_CURHP] &&
		borg_skill[BI_ATELEPORT] <= 0 &&
         (borg_quaff_potion(SV_POTION_STAR_HEALING) ||
          borg_quaff_potion(SV_POTION_HEALING) ||
          borg_quaff_potion(SV_POTION_LIFE)))
    {
    ...
    if (borg_skill[BI_ISCUT] && (borg_skill[BI_CURHP] < borg_skill[BI_MAXHP]/3 || randint1(100) < 20) )
    {
...
bool borg_caution(void)
{
        ...
        (goal_fleeing || scaryguy_on_level || (p > avoidance && borg_skill[BI_CLEVEL] < 35)))
    {
        ...
        {
            borg_desperate = TRUE;
            if (borg_flow_stair_more(GOAL_FLEE, FALSE))
            {
                /* Note */
		...
		!(cave_info[c_y][c_x] & CAVE_ICKY))
   {
    ...
    if (((p > (avoidance *4/10) && !nasty && !borg_no_retreat) || (borg_surround && p != 0)) &&
        !borg_morgoth_position && !borg_skill[BI_ISCONFUSED] &&
		!(cave_info[c_y][c_x] & CAVE_ICKY))
    {
        int i = -1, b_i = -1;
        int k = -1, b_k = -1;
    ...
    {
        /* Flee from low hit-points */
        if (borg_skill[BI_CDEPTH] && (randint1(100) < 25))
        {
            /* Start leaving */
            if (!goal_leaving)
            {
...
static int borg_launch_bolt_aux_hack(int i, int dam, int typ, int ammo_location)
{
	...
	/* Extra Damage if the monster is adjacent to me */
	if (distance(kill->y, kill->x, c_y, c_x) == 1) d = d * 2;
    ...
    for (dist = 0; dist < max; dist++)
    {
        ...
        if (!borg_skill[BI_ESP])
        {
            ...
            if (dist && (borg_skill[BI_INFRA] <=0)
              && !rf_has(r_ptr->flags, RF_HAS_LITE))
            {
    ...
    for (i = 0; i < borg_temp_n; i++)
    {
        ...
        for (o_x = -1; o_x <= 1; o_x++)
        {
            for (o_y = -1; o_y <= 1; o_y++)
            {
                ...
                if ((num > 1) && (randint1(num) != 0)) continue;
...
static int borg_attack_aux_prayer_bolt(int book, int what, int rad, int dam, int typ)
{
    ...
    /* Paranoia */
    if (borg_simulate && (randint1(100) < 2)) return (0);
...
static int borg_attack_aux_rod_bolt(int sval, int rad, int dam, int typ)
{
	...
	if (borg_skill[BI_DEV] - borg_items[borg_slot(TV_ROD, sval)].level <= 10) return (0);
...
static int borg_attack_aux_wand_bolt(int sval, int rad, int dam, int typ)
{
	...
	/* Not likely to be successful in the activation */
	if (borg_skill[BI_DEV] - borg_items[i].level <= 10) return (0);
...
static int borg_attack_aux_wand_bolt_unknown(int dam, int typ)
{
    ...
    /* Paranoia */
    if (borg_simulate && (randint1(100) < 5)) return (0);
...
bool borg_munchkin_mage(void)
{
	...
	/* Define a few low level magic abilities */
	n[0] = BF_SPELL_MAGIC_MISSILE;
	n[1] = BF_WAND_MAGIC_MISSILE;
	n[2] = BF_OBJECT;
	n[3] = BF_LAUNCH_NORMAL;
	n[4] = BF_WAND_UNKNOWN;
...
bool borg_munchkin_melee(void)
{
	...
	if (borg_danger(c_y, c_x, 1, TRUE, FALSE) > avoidance * 2) return (FALSE);
...
static int borg_defend_aux_teleportlevel( int p1)
{
    ...
    if (borg_fighting_unique >= 10 || morgoth_on_level) return (0);
...
static int borg_defend_aux_inviso(int p1)
{
    ...
    /* no need */
	if (borg_skill[BI_ISFORGET] || borg_skill[BI_ISBLIND] || borg_skill[BI_ISCONFUSED] || borg_skill[BI_SINV] || borg_see_inv)
        return (0);
    ...
    /* snap shot */
    if (borg_spell_fail(2, 6, fail_allowed) ||
...
if ( (!borg_skill[BI_ISBLIND] && !borg_skill[BI_ISPOISONED] && !borg_skill[BI_ISCUT] &&
          ...
          !borg_skill[BI_ISWEAK] && !borg_skill[BI_ISHUNGRY] && p < avoidance/5) &&
         (borg_skill[BI_ISCONFUSED] || borg_skill[BI_ISIMAGE] || borg_skill[BI_ISAFRAID] || borg_skill[BI_ISSTUN] || borg_skill[BI_ISHEAVYSTUN] ||
          borg_skill[BI_CURHP] < borg_skill[BI_MAXHP] || borg_skill[BI_CURSP] < borg_skill[BI_MAXSP] * 6 / 10)  &&
         borg_check_rest(c_y, c_x) && !scaryguy_on_level &&
    ...
    if (borg_skill[BI_MAXSP] && borg_skill[BI_CLEVEL] <= 10 &&
...
static bool borg_play_step(int y2, int x2)
{
    borg_grid *ag;
    borg_grid *ag2;
	ui_event_data ch_evt = EVENT_EMPTY;
    ...
    if (ag->take)
    {
            ...
            !strstr(k_info[take->k_idx].name, "Ruined"))
        {
            ...
            if (o_ptr->pval >= 1 && object_is_known(o_ptr) &&
		 ...
		 */
		if (take->orbed == FALSE &&
			(take->k_idx >=lookup_kind(TV_SWORD, SV_DAGGER) && take->k_idx < lookup_kind(TV_SCROLL, SV_SCROLL_PHASE_DOOR)))
		{
    ...
    if ((ag->feat >= FEAT_DOOR_HEAD) && (ag->feat <= FEAT_DOOR_HEAD + 0x07))
    {
        ...
        if (!randint1(100) || time_this_panel >= 500)
		{
    ...
    if (ag->feat >= FEAT_SECRET && ag->feat <= FEAT_WALL_SOLID)
    {
		...
		if ((ag->feat >= FEAT_MAGMA && ag->feat <= FEAT_QUARTZ_H && borg_skill[BI_CLEVEL] <= 30) ||
        ...
        if (randint1(100) < 10) return (FALSE);
        ...
        if (borg_danger(c_y,c_x,1, TRUE, FALSE) >= borg_skill[BI_CURHP] /4) return (FALSE);
...
bool borg_flow_stair_both(int why, bool sneak)
{
    ...
    if (!goal_fleeing && !scaryguy_on_level && (avoidance <= borg_skill[BI_CURHP] * 15/10) &&
 ...
 */
bool borg_flow_stair_more(int why, bool sneak)
{
    int i;
    ...
    /* not unless safe or munchkin/Lunal Mode */
    if (!borg_lunal_mode && !borg_munchkin_mode && (cptr)NULL != borg_prepared(borg_skill[BI_CDEPTH] + 1))
        return (FALSE);
...
bool borg_flow_take_lunal(bool viewable, int nearness)
{
    ...
    for (i = 1; i < borg_takes_nxt; i++)
    {
		...
		if (!borg_items[INVEN_MAX_PACK-1].iqty && item_bad == TRUE)
		{
			...
			if (take->k_idx >= lookup_kind(TV_POTION, SV_POTION_INC_STR) &&
				take->k_idx <= lookup_kind(TV_POTION, SV_POTION_DETECT_INVIS))
			{
				borg_note(format("# Lunal Item %s, at %d,%d", k_info[take->k_idx].name, y, x ));
				item_bad = FALSE;
			}
			 ...
			 */
			if (take->k_idx >= lookup_kind(TV_LIGHT, SV_PHIAL))
			{
				borg_note(format("# Lunal Item %s, at %d,%d", k_info[take->k_idx].name, y, x ));
				item_bad = FALSE;
			}
