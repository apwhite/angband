...
void borg_log_death(void)
{
   ...
   /* Append to the file */
   borg_log_file = file_open(buf, MODE_WRITE, FTYPE_TEXT);
...
void borg_log_death_data(void)
{
   ...
   file_putf(borg_log_file, "%s, %s, %s, %d, %d, %s\n",borg_engine_date, p_info[p_ptr->prace].name,
   c_info[p_ptr->pclass].name, p_ptr->lev, p_ptr->depth, p_ptr->died_from);
...
static bool borg_think(void)
{
		 ...
		 streq(buf, "Home")))
    {
		...
		shop_num = (cave_feat[p_ptr->py][p_ptr->px] - FEAT_SHOP_HEAD);
...
static int adjust_stat_borg(int value, int amount, int borg_roll)
{
    ...
    if ((amount < 0) || op_ptr->opt[OPT_adult_maximize])
    {
...
static void get_stats_borg_aux(void)
{
    ...
    for (i = 0; i < A_MAX; i++)
    {
        ...
        /* Variable stat maxes */
        if (op_ptr->opt[OPT_adult_maximize])
        {
            /* Start fully healed */
...
static void create_random_name(int race, char *name)
{
    ...
    case RACE_DWARF:
        strcpy(name, dwarf_syllable1[randint1(sizeof(dwarf_syllable1) / sizeof(char*))]);
        strcat(name, dwarf_syllable2[randint1(sizeof(dwarf_syllable2) / sizeof(char*))]);
        strcat(name, dwarf_syllable3[randint1(sizeof(dwarf_syllable3) / sizeof(char*))]);
        break;
    case RACE_ELF:
    case RACE_HALF_ELF:
    case RACE_HIGH_ELF:
        strcpy(name, elf_syllable1[randint1(sizeof(elf_syllable1) / sizeof(char*))]);
        strcat(name, elf_syllable2[randint1(sizeof(elf_syllable2) / sizeof(char*))]);
        strcat(name, elf_syllable3[randint1(sizeof(elf_syllable3) / sizeof(char*))]);
        break;
    case RACE_GNOME:
        strcpy(name, gnome_syllable1[randint1(sizeof(gnome_syllable1) / sizeof(char*))]);
        strcat(name, gnome_syllable2[randint1(sizeof(gnome_syllable2) / sizeof(char*))]);
        strcat(name, gnome_syllable3[randint1(sizeof(gnome_syllable3) / sizeof(char*))]);
        break;
    case RACE_HOBBIT:
        strcpy(name, hobbit_syllable1[randint1(sizeof(hobbit_syllable1) / sizeof(char*))]);
        strcat(name, hobbit_syllable2[randint1(sizeof(hobbit_syllable2) / sizeof(char*))]);
        strcat(name, hobbit_syllable3[randint1(sizeof(hobbit_syllable3) / sizeof(char*))]);
        break;
    case RACE_HUMAN:
    case RACE_DUNADAN:
        strcpy(name, human_syllable1[randint1(sizeof(human_syllable1) / sizeof(char*))]);
        strcat(name, human_syllable2[randint1(sizeof(human_syllable2) / sizeof(char*))]);
        strcat(name, human_syllable3[randint1(sizeof(human_syllable3) / sizeof(char*))]);
        break;
    case RACE_HALF_ORC:
    case RACE_HALF_TROLL:
    case RACE_KOBOLD:
        strcpy(name, orc_syllable1[randint1(sizeof(orc_syllable1) / sizeof(char*))]);
        strcat(name, orc_syllable2[randint1(sizeof(orc_syllable2) / sizeof(char*))]);
        strcat(name, orc_syllable3[randint1(sizeof(orc_syllable3) / sizeof(char*))]);
        break;
...
void resurrect_borg(void)
{
    ...
    {
        p_ptr->pclass = randint0(z_info->c_max);
    }
    else
    {
   ...
   {
	   /* build in a loop check */
	   i = 0;

    while (i < 1000)
    {
       /* Race */
       p_ptr->prace = randint0(z_info->p_max);
   ...
   else
   {
    ...
    /* outfit the player */
	p_ptr->inventory = C_ZNEW(ALL_INVEN_TOTAL, struct object);
    ...
    my_oppose_acid = (p_ptr->timed[TMD_OPP_ACID] ? TRUE : FALSE);
    my_oppose_elec = (p_ptr->timed[TMD_OPP_ELEC] ? TRUE : FALSE);
    my_oppose_fire = (p_ptr->timed[TMD_OPP_FIRE] ? TRUE : FALSE);
    my_oppose_cold = (p_ptr->timed[TMD_OPP_COLD] ? TRUE : FALSE);
    my_oppose_pois = (p_ptr->timed[TMD_OPP_POIS] ? TRUE : FALSE);
...
static char borg_inkey_hack(int flush_first)
{
	...
	if (!borg_in_shop && (ch_evt.type & EVT_KBRD) && ch_evt.key > 0 &&
		ch_evt.key != 10)
	{
		/* Oops */
		borg_note(format("# User key press <%d><%c>",ch_evt.key,ch_evt.key));
...
static bool borg_load_formula(char * string)
{
	...
	formula[formula_num] = C_ZNEW(MAX_FORMULA_ELEMENTS, int);
...
void init_borg_txt_file(void)
{
    ...
    {
		borg_required_item[i] = C_ZNEW(400, req_item);
        n_req[i] = 0;
		borg_power_item[i] = C_ZNEW(400, power_item);
        n_pwr[i] = 0;
    }
    for (i = 0; i < 999; i++)
    {
...
void borg_init_9(void)
{
    ...
    test = C_ZNEW(400 * 1024L, byte);
	FREE(test);


    /*** Hack -- initialize some stuff ***/
	borg_required_item = C_ZNEW(z_info->c_max, req_item*);
	n_req= C_ZNEW(z_info->c_max, int);
	borg_power_item = C_ZNEW(z_info->c_max, power_item*);
	n_pwr = C_ZNEW(z_info->c_max, int);
...
void do_cmd_borg(void)
{
    ...
    switch (cmd)
    {
		...
		case 'G':
		{
			...
			borg_note(format("Borg's Feat for grid (%d, %d) is %d, game Feat is %d", y,x,mask, cave_feat[y][x]));
			prt_map();
			break;
		}
		...
		case 'I':
		{
			...
			x = tx;
				...
				if (borg_grids[y][x].info & BORG_MARK) msg_format("Info for grid (%d, %d) is MARK", y,x);
                if (borg_grids[y][x].info & BORG_GLOW) msg_format("Info for grid (%d, %d) is GLOW", y,x);
                if (borg_grids[y][x].info & BORG_DARK) msg_format("Info for grid (%d, %d) is DARK", y,x);
                if (borg_grids[y][x].info & BORG_OKAY)	msg_format("Info for grid (%d, %d) is OKAY", y,x);
                if (borg_grids[y][x].info & BORG_LIGHT)	msg_format("Info for grid (%d, %d) is LITE", y,x);
                if (borg_grids[y][x].info & BORG_VIEW)	msg_format("Info for grid (%d, %d) is VIEW", y,x);
                if (borg_grids[y][x].info & BORG_TEMP)	msg_format("Info for grid (%d, %d) is TEMP", y,x);
                if (borg_grids[y][x].info & BORG_XTRA)	msg_format("Info for grid (%d, %d) is XTRA", y,x);
				if (cave_info[y][x] & CAVE_ICKY) msg_format("Info for grid (%d, %d) is ICKY", y,x);

				borg_note(format("Info for grid (%d, %d) is %d", y,x,cave_info[y][x]));
			prt_map();
			break;
		}
    ...
    /* Command: check previous steps */
        case 'y':
        {
            ...
            /* Scan map */
                    ...
                    for (i = 0; i < track_step_num; i++)
                    {
			            ...
			            msg_format("(-%d) Steps noted %d,%d", i,track_step_y[track_step_num-i], track_step_x[track_step_num-i]);
						msg_print(NULL);
        ...
        case '^':
        {
                ...
                for (o = 0; o < 250; o++)
                {
                    ...
                    for (i = 0; i < 8; i++)
                    {
                        ...
                        if ((++b_n >= 2) && (randint1(b_n) != 0)) continue;
        ...
        case '@':
        {
            ...
            /* Get keypress */
            msg_format("Borg has Projectable to these places.");
            msg_print(NULL);
       ...
       case 'm':
        ...
        case ';':
        {
	        ...
	        for (glyph_check = 0; glyph_check < track_glyph_num; glyph_check++)
		    {
					...
					msg_format("Borg has Glyph (%d)noted.", glyph_check);
		            msg_print(NULL);
            }
