...
static void borg_notice_aux2(void)
{
	...
	/* Activatable equipment HASTE */
		...
		(!borg_has[lookup_kind(TV_ROD, SV_ROD_SPEED)]))
	{
...
cptr borg_restock(int depth)
{
	...
	if (borg_munchkin_start && borg_skill[BI_MAXCLEVEL] < borg_munchkin_level) return ((cptr)NULL);
...
cptr borg_prepared(int depth)
{
      ...
      /* unless all the uniques are dead. */
    {
		...
		if (borg_living_unique_index == 546) return ((cptr)NULL);
