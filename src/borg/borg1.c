...
void borg_update_frame(void)
{
    ...
    if (!my_oppose_fire && p_ptr->timed[TMD_OPP_FIRE]) my_oppose_fire = (p_ptr->timed[TMD_OPP_FIRE] ? TRUE : FALSE);
    if (!my_oppose_cold && p_ptr->timed[TMD_OPP_COLD]) my_oppose_cold = (p_ptr->timed[TMD_OPP_COLD] ? TRUE : FALSE);
    if (!my_oppose_pois && p_ptr->timed[TMD_OPP_POIS]) my_oppose_pois = (p_ptr->timed[TMD_OPP_POIS] ? TRUE : FALSE);
    if (!borg_bless && p_ptr->timed[TMD_BLESSED]) borg_bless = (p_ptr->timed[TMD_BLESSED] ? TRUE : FALSE);
    if (!borg_shield && p_ptr->timed[TMD_SHIELD]) borg_shield = (p_ptr->timed[TMD_SHIELD] ? TRUE : FALSE);
    if (!borg_hero && p_ptr->timed[TMD_HERO]) borg_hero = (p_ptr->timed[TMD_HERO] ? TRUE : FALSE);
    if (!borg_berserk && p_ptr->timed[TMD_SHERO]) borg_berserk = (p_ptr->timed[TMD_SHERO] ? TRUE : FALSE);
...
void borg_init_1(void)
{
	...
	borg_key_queue = C_ZNEW(KEY_SIZE, char);
...
if (!borg_rand_local)
    ...
    {
        /* Make each row */
		borg_grids[y] = C_ZNEW(AUTO_MAX_X, borg_grid);
    }
    ...
    /* Allocate */
    borg_data_flow = ZNEW(borg_data);

    /* Allocate */
    borg_data_cost = ZNEW(borg_data);

    /* Allocate */
    borg_data_hard = ZNEW(borg_data);

    /* Allocate */
    borg_data_know = ZNEW(borg_data);

    /* Allocate */
    borg_data_icky = ZNEW(borg_data);
	...
	track_shop_x = C_ZNEW(9, int);
	track_shop_y = C_ZNEW(9, int);
    ...
    track_less_num = 0;
    track_less_size = 16;
	track_less_x = C_ZNEW(track_less_size, int);
	track_less_y = C_ZNEW(track_less_size, int);

    /* Track "down" stairs */
    track_more_num = 0;
    track_more_size = 16;
	track_more_x = C_ZNEW(track_more_size, int);
	track_more_y = C_ZNEW(track_more_size, int);

    /* Track glyphs */
    track_glyph_num = 0;
    track_glyph_size = 200;
	track_glyph_x = C_ZNEW(track_glyph_size, int);
	track_glyph_y = C_ZNEW(track_glyph_size, int);

    /* Track the worn items to avoid loops */
    track_worn_num = 0;
    track_worn_size = 10;
	track_worn_time = 0;
	track_worn_name1 = C_ZNEW(track_worn_size, byte);

    /* Track Steps */
    track_step_num = 0;
    track_step_size = 100;
	track_step_x = C_ZNEW(track_step_size, int);
	track_step_y = C_ZNEW(track_step_size, int);

    /* Track doors closed by borg */
    track_door_num = 0;
    track_door_size = 100;
	track_door_x = C_ZNEW(track_door_size, int);
	track_door_y = C_ZNEW(track_door_size, int);

    /* Track closed doors on map */
    track_closed_num = 0;
    track_closed_size = 100;
	track_closed_x = C_ZNEW(track_closed_size, int);
	track_closed_y = C_ZNEW(track_closed_size, int);

    /* Track mineral veins with treasure. */
    track_vein_num = 0;
    track_vein_size = 100;
	track_vein_x = C_ZNEW(track_vein_size, int);
	track_vein_y = C_ZNEW(track_vein_size, int);
    ...
    /* Array of objects */
	borg_takes = C_ZNEW(256, borg_take);
    ...
    /* Array of monsters */
	borg_kills = C_ZNEW(256, borg_kill);
	...
	borg_race_count = C_ZNEW(z_info->r_max, s16b);

    /* Count racial deaths */
	borg_race_death = C_ZNEW(z_info->r_max, s16b);
