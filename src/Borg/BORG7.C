...
bool borg_crush_junk(void)
{
    ...
    for (i = 0; i < INVEN_MAX_PACK; i++)
    {
        ...
        /*         so check them all */
        if ((op_ptr->opt[OPT_birth_randarts] || op_ptr->opt[OPT_adult_randarts]) && item->name1 && !item->fully_identified) continue;
...
bool borg_crush_slow(void)
{
    ...
    for (i = 0; i < ALL_INVEN_TOTAL; i++)
    {
		...
		if (i >= INVEN_WIELD && i <= INVEN_FEET) continue;
...
bool borg_test_stuff(void)
{
    ...
    for (i = 0; i < INVEN_MAX_PACK; i++)
    {
        ...
        switch (item->tval)
        {
            ...
            if ((borg_class == CLASS_MAGE) && (randint1(1000) < borg_skill[BI_CLEVEL]))
            {
            ...
            if ((borg_class == CLASS_RANGER) && (randint1(3000) < borg_skill[BI_CLEVEL]))
            {
            ...
            if ((borg_class == CLASS_PRIEST) && (randint1(5000) < borg_skill[BI_CLEVEL]))
            {
    ...
    if (b_i >= 0)
    {
        borg_item *item = &borg_items[b_i];
            ...
            if (borg_spell(2, 5) ||
                ...
                borg_activate_artifact(EFF_IDENTIFY, INVEN_WIELD) ||
...
bool borg_wear_rings(void)
{
    ...
    for (i = 0; i < INVEN_MAX_PACK; i++)
    {
        ...
        if ((op_ptr->opt[OPT_birth_randarts] || op_ptr->opt[OPT_adult_randarts]) && !item->fully_identified && item->name1) continue;
        ...
        if (randint1(100) > 75 || item->activation == EFF_BIZARRE)
        {
            slot = INVEN_RIGHT;
        }
...
bool borg_backup_swap(int p)
{
    ...
    if ((op_ptr->opt[OPT_birth_randarts] || op_ptr->opt[OPT_adult_randarts]) && !item->fully_identified && item->name1) b_p1 = 9999;
    ...
    /*  skip random artifact not star id'd  */
    if ((op_ptr->opt[OPT_birth_randarts] || op_ptr->opt[OPT_adult_randarts]) && !item->fully_identified && item->name1) b_p2 = 9999;
...
bool borg_dump_quiver(void)
{
    ...
    for (i = QUIVER_START; i < QUIVER_END; i++)
    {
        ...
        if (!item->cursed && item->tval == my_ammo_tval && 
			(item->to_d >= 0 && item->to_h >=0)) continue;
...
bool borg_wear_stuff(void)
{
    ...
    for (i = 0; i < INVEN_MAX_PACK; i++)
    {
        ...
        if ((op_ptr->opt[OPT_birth_randarts] || op_ptr->opt[OPT_adult_randarts]) &&
	...
	if (randint1(100)==10 || item->activation == EFF_BIZARRE)
	{
        /* ring, full hands */
        if (slot == INVEN_LEFT &&
            borg_items[INVEN_LEFT].tval && borg_items[INVEN_RIGHT].tval)
...
bool borg_wear_quiver(void)
{
    ...
    for (i = 0; i < INVEN_MAX_PACK; i++)
    {
		...
		/* Load these missiles into the quiver */
		b_i = i;
    ...
    for (k = 0; k < 12; k++)
    {
        ...
        else
        {
            ...
            if (sold_item_tval == item->tval && sold_item_sval == item->sval &&
                sold_item_store == 7) return (FALSE);
    ...
    if (!borg_skill[BI_CDEPTH])
    {
        ...
        if (borg_skill[BI_MAXDEPTH] == 100 && !borg_plays_risky)
        {
	        if (borg_skill[BI_ATELEPORTLVL] == 0)
            {
                ...
                if (borg_flow_stair_more(GOAL_BORE, FALSE)) return (TRUE);
