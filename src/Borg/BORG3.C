...
void borg_item_analyze(borg_item *item, object_type *real_item, cptr desc)
{
	...
	if (!item->aware)
	{
		if (object_flavor_is_aware(real_item)) item->kind = real_item->k_idx;
	...
	/* Item has been ID'd (store, scroll, spell) */
  	if ((real_item->ident & IDENT_KNOWN) ||
		...
		(real_item->ident & IDENT_DEFENCE) ||
  	    (real_item->ident & IDENT_STORE)) item->ident = TRUE;
    ...
    if (item->aware) item->kind = real_item->k_idx;
...
/*     item->discount = real_item->discount */
	 ...
	 */
	if (item->kind == lookup_kind(TV_LIGHT, SV_PLANATIR))
	{
	...
	if (!item->activation)
	{
		item->activation = k_info[real_item->k_idx].effect;
	}
...
bool borg_activate_artifact(int activation, int location)
{
    ...
    for (i = INVEN_WIELD; i < INVEN_TOTAL; i++)
    {
         ...
         */
        if  ((op_ptr->opt[OPT_birth_randarts] || op_ptr->opt[OPT_adult_randarts]) && (item->activation != EFF_ILLUMINATION &&
...
bool borg_inscribe_food(void)
{
    ...
    for (ii=0; ii < INVEN_TOTAL; ii++)
    {
        ...
        if (item->sval == SV_FOOD_SLIME_MOLD || item->sval == SV_FOOD_RATION)
        {
            ...
            {
                /* get a name */
                strcpy(name, food_syllable1[randint1(sizeof(food_syllable1) / sizeof(char*))]);
                strcat(name, food_syllable2[randint1(sizeof(food_syllable2) / sizeof(char*))]);
            ...
            {
                /* get a name */
                strcpy(name, mold_syllable1[randint1(sizeof(mold_syllable1) / sizeof(char*))]);
                strcat(name, mold_syllable2[randint1(sizeof(mold_syllable2) / sizeof(char*))]);
                strcat(name, mold_syllable3[randint1(sizeof(mold_syllable3) / sizeof(char*))]);
...
void borg_cheat_equip(void)
{
    ...
    for (i = INVEN_WIELD; i < ALL_INVEN_TOTAL; i++)
    {
        ...
        if (p_ptr->inventory[i].ident & IDENT_KNOWN)
        {
...
void borg_cheat_store(void)
{
	...
	for (shop_num = 0; shop_num < MAX_STORES;  shop_num ++)
	{
		...
		for (slot = 0; slot < 24; slot++)
		{
			...
			/* Analyze the item */
			borg_item_analyze(&borg_shops[shop_num].ware[slot], &store[shop_num].stock[slot], buf);
			...
			if (store[shop_num].stock[slot].ident & IDENT_KNOWN)
			{
			...
			if (shop_num == 7) borg_shops[shop_num].ware[slot].cost = 0;
			else borg_shops[shop_num].ware[slot].cost = price_item(j_ptr, FALSE,1);
		}
	}
}
...
void borg_init_3(void)
{
	...
	borg_items = C_ZNEW(QUIVER_END, borg_item);

    /* Make the stores in the town */
	borg_shops = C_ZNEW(9, borg_shop);

    /*** Item/Ware arrays (simulation) ***/

    /* Make the "safe" inventory array */
	safe_items = C_ZNEW(QUIVER_END, borg_item);
	safe_home = C_ZNEW(STORE_INVEN_MAX, borg_item);

    /* Make the "safe" stores in the town */
	safe_shops = C_ZNEW(8, borg_shop);
	...
	borg_sv_plural_text = C_ZNEW(z_info->k_max, cptr);
	...
	borg_plural_text = C_ZNEW(borg_plural_size, cptr);
	borg_plural_what = C_ZNEW(borg_plural_size, s16b);
    ...
    /* Allocate the "item parsing arrays" (plurals) */
    borg_single_text = C_ZNEW(borg_single_size, cptr);
    borg_single_what = C_ZNEW(borg_single_size, s16b);
	...
	borg_sv_art_text = C_ZNEW(z_info->a_max, cptr); 
    ...
    /* Allocate the "item parsing arrays" (plurals) */
	borg_artego_text = C_ZNEW( borg_artego_size, cptr); 
	borg_artego_what = C_ZNEW( borg_artego_size, s16b); 
