...
static void borg_notice_aux2(void)
{
    ...
    for (i = 0; i < INVEN_MAX_PACK; i++)
    {
        ...
        switch (item->tval)
        {
            ...
            switch (item->sval)
            {
                ...
                if (borg_skill[BI_DEV] - item->level > 7)
                {
					borg_skill[BI_ATELEPORT] += item->pval;
				}
                break;

                case SV_STAFF_SPEED:
                borg_skill[BI_ASPEED] += item->pval;
                break;
    ...
    if (borg_spell_legal_fail(2, 0, 25) || borg_prayer_legal_fail(1, 5, 25))
    {
    ...
    if (my_stat_cur[A_STR] >= (18+100) + 10 * op_ptr->opt[OPT_adult_maximize] *
        (rp_ptr->r_adj[A_STR] + cp_ptr->c_adj[A_STR]))
        amt_add_stat[A_STR] += 1000;

    if (my_stat_cur[A_INT] >= (18+100) + 10 * op_ptr->opt[OPT_adult_maximize] *
        (rp_ptr->r_adj[A_INT] + cp_ptr->c_adj[A_INT]))
         amt_add_stat[A_INT] += 1000;

    if (my_stat_cur[A_WIS] >= (18+100) + 10 * op_ptr->opt[OPT_adult_maximize] *
        (rp_ptr->r_adj[A_WIS] + cp_ptr->c_adj[A_WIS]))
        amt_add_stat[A_WIS] += 1000;

    if (my_stat_cur[A_DEX] >= (18+100) + 10 * op_ptr->opt[OPT_adult_maximize] *
        (rp_ptr->r_adj[A_DEX] + cp_ptr->c_adj[A_DEX]))
         amt_add_stat[A_DEX] += 1000;

    if (my_stat_cur[A_CON] >= (18+100) + 10 * op_ptr->opt[OPT_adult_maximize] *
        (rp_ptr->r_adj[A_CON] + cp_ptr->c_adj[A_CON]))
        amt_add_stat[A_CON] += 1000;

    if (my_stat_cur[A_CHR] >= (18+100) + 10 * op_ptr->opt[OPT_adult_maximize] *
    ...
    for (i = 0; i < INVEN_MAX_PACK; i++)
    {
       ...
       /* Clear all the swap weapon flags as I look at each one. */
        ...
        switch (item->tval)
        {
            ...
            if (borg_skill[BI_MAXDEPTH] <= 40 && borg_skill[BI_MAXDEPTH] >= 25 && weapon_swap_digger) v += (weapon_swap_digger * 3500L) + 1000L;
...
static s32b borg_power_aux1(void)
{
    ...
    for (i = INVEN_WIELD; i < INVEN_TOTAL; i++)
    {
        ...
        multibonus = (of_has(item->flags, OF_RES_POIS) +
               of_has(item->flags, OF_RES_LIGHT) +
               of_has(item->flags, OF_RES_DARK) +
        ...
        if (!op_ptr->opt[OPT_adult_randarts] &&
...
static s32b borg_power_aux2(void)
{
        ...
        borg_class == CLASS_MAGE)
    {
        ...
        k = 0; /* carry a couple for emergency. Store the rest. */
        for (; k < 1 && k < borg_skill[BI_AEZHEAL]; k++) value +=9000L;
        ...
        /* These guys need to carry the rods more, town runs low on supply. */
        k = 0;
        for (; k < 2 && k < borg_has[ROD_HEAL]; k++) value +=20000L;
    ...
    else if (borg_class == CLASS_PRIEST)
    {
        ...
        k = 0; /* carry a couple for emergency. Store the rest. */
        for (; k < 1 && k < borg_has[POTION_STAR_HEAL]; k++) value +=9000L;
    }
    ...
    if (borg_skill[BI_MAXDEPTH] >= 99)
    {
        ...
        if (borg_race_death[546] != 0)
        {
            ...
            else
            {
                ...
                for (; k < 99 && k < borg_skill[BI_ASPEED]; k++) value +=8000L;
                k = 0;
                for (; k < 99 && k < borg_has[POTION_RES_MANA]; k++) value += 5000L; /* Restore Mana */
    ...
    if (borg_skill[BI_ACCW] < 5 && (borg_skill[BI_CLEVEL] < 35 || !borg_skill[BI_RCONF]))
    {
        k = 0;
    ...
    if (borg_skill[BI_MAXDEPTH] <= 40 && borg_skill[BI_MAXDEPTH] >= 25 && borg_items[INVEN_WIELD].tval != TV_DIGGING   &&
    ...
    if (inven_weight > carry_capacity/2)
   	{
		...
		if (item->iqty &&
			...
			(item->tval == TV_POTION &&
			 (item->sval == SV_POTION_RES_STR ||
			  item->sval == SV_POTION_RES_INT ||
			  item->sval == SV_POTION_RES_WIS ||
			  item->sval == SV_POTION_RES_DEX ||
			  item->sval == SV_POTION_RES_CON ||
			  item->sval == SV_POTION_RES_CHR ||
...
static s32b borg_power_home_aux2(void)
{
    ...
    for (k = 0; k < 90 && k < num_mass_genocide; k++) value += 5000L;
	...
	if (q >= 20)
	{
		...
		for (ii = 0; ii < 8; ii++)
		{
				  ...
				  borg_grids[y_temp][x_temp].feat == FEAT_RUBBLE))
			{
			    ...
			    if (rf_has(r_ptr->flags, RF_PASS_WALL))
				{
					if (borg_projectable(y_temp, x_temp, y, x)) b_v2 = v2 * q / 10;
				}
				if (rf_has(r_ptr->flags, RF_KILL_WALL))
				{
					if (borg_projectable(y_temp, x_temp, y, x)) b_v2 = v2 * q / 10;
			...
			else if (borg_projectable(y_temp, x_temp, y, x)) b_v2 = v2 * q / 10;
		}
...
int borg_danger(int y, int x, int c, bool average, bool full_damage)
{
    ...
    if (time_this_panel <= 200 &&
		!(cave_info[y][x] & (CAVE_ICKY))) p += borg_fear_monsters[y][x] * c;
...
cptr borg_restock(int depth)
{
    ...
    if (borg_skill[BI_ACLW] + borg_skill[BI_ACSW] + borg_skill[BI_ACCW]< 1) return ("rs clw/csw");
